...
public partial class HtmlDataGrid<TGridItem> : ComponentBase, IAsyncDisposable
{
    ...
    /// <summary>
    /// Optionally links this <see cref="HtmlDataGrid{TGridItem}"/> instance with a <see cref="PaginationState"/> model,
    ...
    /// <summary>
    /// Constructs an instance of <see cref="HtmlDataGrid{TGridItem}"/>.
    /// </summary>
    public HtmlDataGrid()
    {
        _columns = new();
        ...
        var columnsFirstCollectedSubscriber = new EventCallbackSubscriber<object?>(EventCallback.Factory.Create<object?>(this, RefreshDataCoreAsync));
    ...
    protected override Task OnParametersSetAsync()
    {
        ...
        {
            throw new InvalidOperationException($"{nameof(HtmlDataGrid<TGridItem>)} requires one of {nameof(Items)} or {nameof(ItemsProvider)}, but both were specified.");
        }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _jsModule = await JS.InvokeAsync<IJSObjectReference>("import", "./_content/DSE.Open.RazorToolkit.UI.Web.Components.Html/Controls/HtmlDataGrid.razor.js");
...
#pragma warning restore CA2012 // Use ValueTasks correctly
    ...
    private async Task RefreshDataCoreAsync()
    {
        ...
        else
        {
            ...
            var request = new GridItemsProviderRequest<TGridItem>(startIndex, Pagination?.ItemsPerPage, _sortByColumn, _sortByAscending, thisLoadCts.Token);
    ...
    private async ValueTask<ItemsProviderResult<(int, TGridItem)>> ProvideVirtualizedItems(ItemsProviderRequest request)
    {
        ...
        var providerRequest = new GridItemsProviderRequest<TGridItem>(startIndex, count, _sortByColumn, _sortByAscending, request.CancellationToken);
        ...
        if (!request.CancellationToken.IsCancellationRequested)
        {
            ...
            return new ItemsProviderResult<(int, TGridItem)>(items: providerResult.Items.Select((x, i) => ValueTuple.Create(i + request.StartIndex + 2, x)), totalItemCount: _ariaBodyRowCount);
        }
    ...
    private string GridClass() => $"quickgrid {Class} {(_pendingDataLoadCancellationTokenSource is null ? null : "loading")}";
