...
// One awkwardness of the way Grid collects its list of child columns is that, during OnParametersSetAsync,
...
// - Or, we could factor out most of Grid's internals into some new component GridCore. The parent component,
//   Grid, would then only be responsible for collecting columns followed by rendering GridCore. So each time
//   GridCore renders, we'd already have the latest set of columns
//    - Drawback: since Grid has public API, it's much messier to have to forward all of that to some new child type.
