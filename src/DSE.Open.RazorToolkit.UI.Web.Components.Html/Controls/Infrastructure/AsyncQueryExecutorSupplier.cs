...
internal static class AsyncQueryExecutorSupplier
{
    ...
    //  - ... but without Grid referencing Microsoft.EntityFramework directly. That's because it would bring in
    ...
    public static IAsyncQueryExecutor? GetAsyncQueryExecutor<T>(IServiceProvider services, IQueryable<T>? queryable)
    {
        ...
        if (executor is null)
        {
            ...
            {
                throw new InvalidOperationException($"The supplied {nameof(IQueryable)} is provided by Entity Framework. To query it efficiently, you must reference the package Microsoft.AspNetCore.Components.Grid.EntityFrameworkAdapter and call AddGridEntityFrameworkAdapter on your service collection.");
            }
        }
    ...
    private static bool IsEntityFrameworkProviderType(Type queryableProviderType)
    {
        return queryableProviderType.GetInterfaces().Any(x =>
            string.Equals(x.FullName, "Microsoft.EntityFrameworkCore.Query.IAsyncQueryProvider", StringComparison.Ordinal));
    }
}
