...
/// <summary>
/// Parameters for data to be supplied by a <see cref="HtmlDataGrid{TGridItem}"/>'s <see cref="HtmlDataGrid{TGridItem}.ItemsProvider"/>.
/// </summary>
...
public readonly struct GridItemsProviderRequest<TGridItem>
{
    ...
    {
        return $"The current sort column is of type '{col.GetType().FullName}'," +
               $" which does not implement {nameof(ISortBuilderColumn<TGridItem>)}, " +
               $"so its sorting rules cannot be applied automatically.";
    }
}
